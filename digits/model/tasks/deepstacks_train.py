...
# Constants
DEEPSTACKS_MODEL_FILE = 'network.py'
DEEPSTACKS_SNAPSHOT_PREFIX = 'snapshot'
...
@subclass
class DeepstacksTrainTask(TrainTask):
    """
    Trains a deepstacks model
    """

    DEEPSTACKS_LOG = 'deepstacks_output.log'
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(DeepstacksTrainTask, self).__init__(**kwargs)

        # save network description to file
        with open(os.path.join(self.job_dir, DEEPSTACKS_MODEL_FILE), "w") as outfile:
            outfile.write(self.network)

        self.pickver_task_deepstacks_train = PICKLE_VERSION
        ...
        self.model_file = DEEPSTACKS_MODEL_FILE
        self.train_file = constants.TRAIN_DB
        self.val_file = constants.VAL_DB
        self.snapshot_prefix = DEEPSTACKS_SNAPSHOT_PREFIX
        self.log_file = self.DEEPSTACKS_LOG

    def __getstate__(self):
        state = super(DeepstacksTrainTask, self).__getstate__()
        ...
        if 'deepstacks_log' in state:
            del state['deepstacks_log']

        return state

    def __setstate__(self, state):
        super(DeepstacksTrainTask, self).__setstate__(state)
    ...
    @override
    def name(self):
        return 'Train Deepstacks Model'

    @override
    def before_run(self):
        super(DeepstacksTrainTask, self).before_run()
        self.deepstacks_log = codecs.open(self.path(self.DEEPSTACKS_LOG), 'a','utf-8')
    ...
    def get_snapshot(self, epoch=-1, download=False):
        ...
        if download:
            ...
            snapshot_files = [snapshot_pre + "-model-global-0.npz",snapshot_pre + "-model-layers-0.npz"]
        else:
...
#            args.append('--augHSVv=0')
    ...
    def process_output(self, line):
        self.deepstacks_log.write('%s\n' % line)
        self.deepstacks_log.flush()

        # parse deepstacks output
        timestamp, level, message = self.preprocess_output_deepstacks(line)
...
#
    ...
    @staticmethod
    def preprocess_output_deepstacks(line):
        """
        Takes line of output and parses it according to deepstacks's output format
    ...
    def after_run(self):
        if self.temp_unrecognized_output:
            if self.traceback:
                ...
                self.traceback = '\n'.join(self.temp_unrecognized_output)
                self.temp_unrecognized_output = []
        self.deepstacks_log.close()

    @override
    def after_runtime_error(self):
        if os.path.exists(self.path(self.DEEPSTACKS_LOG)):
            output = subprocess.check_output(['tail', '-n40', self.path(self.DEEPSTACKS_LOG)])
            lines = []
            for line in output.split('\n'):
                # parse deepstacks header
                timestamp, level, message = self.preprocess_output_deepstacks(line)
    ...
    def detect_snapshots(self):
        ...
        for filename in os.listdir(self.job_dir):
            # find models
            match = re.match(r'%s-epoch(\d+)-model-layers-0.npz$' % self.snapshot_prefix, filename)
            if match:
                epoch = 0
                filename = filename[:-len('-model-layers-0.npz')]
...
#                env['CUDA_VISIBLE_DEVICES'] = subprocess_visible_devices([gpu])
    ...
    def get_network_desc(self):
        ...
        """
        with open(os.path.join(self.job_dir, DEEPSTACKS_MODEL_FILE), "r") as infile:
            desc = infile.read()
        return desc
